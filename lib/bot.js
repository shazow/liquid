...
var logger = require('./logger.js'),
    ...
    BitmeExchange = require('./exchanges/bitme.js'),
    BitstampExchange = require('./exchanges/bitstamp.js'),
    BitfinexExchange = require('./exchanges/bitfinex.js'),
    DummyExchange = require('./exchanges/dummy.js');
...
var Bot = module.exports.Bot = function(originExchange, remoteExchange, options) {
    ...
    // Place limit orders at a discount to mimic market orders, default to maxVolatility.
    this.remoteOrderDiscount = options.remoteOrderDiscount !== undefined ? options.remoteOrderDiscount : options.maxVolatility;
...
Bot.fromConfig = function(config) {
    ...
    var exchangeLookup = {};
    Bot.exchanges.map(function(o) { exchangeLookup[o.id] = o; });

    // Confirm valid exchange ids.
    var exchanges = [config.originId, config.remoteId].map(function(id) {
        var exchange = exchangeLookup[id];
    ...
    if (!config.pretend) {
        // Check if we should force pretend mode.
        exchanges.every(function(exchange) {
            if (exchange.forcePretend) {
                logger.info('Exchange (%s) forcing PRETEND mode.', exchange.id);
                config.pretend = true;
                return false;
            }

            var missingKeys = util.missingStringValues(config.apiKeys, exchange.configKeys);
            if (missingKeys.length > 0) {
                logger.warn('Missing API keys for exchange (%s), forcing PRETEND mode.', exchange.id);
                config.pretend = true;
                return false;
            }
            return true;
    ...
    var originExchange = exchanges[0].fromConfig(config, 'origin');
    var remoteExchange = exchanges[1].fromConfig(config, 'remote');
...
Bot.prototype.reset = function(callback) {
    logger.info('Resetting exchanges into a safe state. Currently:', {
        'state': this.state,
        'origin': {
            'id': this.originExchange.id,
            'balance': this.originExchange.toObject().balance
        },
        'remote': {
            'id': this.remoteExchange.id,
            'balance': this.remoteExchange.toObject().balance
        }
    });
    var bot = this;
...
Bot.prototype.handleOriginTrade = function(order, callback) {
    logger.info('[bot:origin] Trade:', order.toString());

    var bot = this;
    var newOrder = order.clone({}, bot.premium, /* invertType */ true);
    if (bot.remoteOrderDiscount) {
        // Apply discount. We double-invert again to apply the premium logic in
        // the opposite direction.
        newOrder = newOrder.clone({}, 1.0 + bot.remoteOrderDiscount, true).clone({}, undefined, true);
    }

    this.remoteExchange.placeOrders([newOrder], function(err, result) {
        if (err) {
            logger.error('Failed to sync remote orderbook, aborting:', err.message, dumpErr(err));
...
Bot.prototype.handleRemoteTrade = function(order, callback) {
    logger.info('[bot:remote] Matched and completed:', order.toString());
...
Bot.prototype.handleRemoteOrderbook = function(orders, callback) {
    ...
    async.series([
        ...
        if (err) {
            logger.error('Failed to sync origin orderbook, aborting:', err.message, dumpErr(err));
            bot.abort(err, callback);
            return;
        }
...
/**
 * Given an error, try to extract some useful metadata from it to include
 * in our logging.
 *
 * @param {Error} err Error object that might have extra attributes.
 * @return {Object}
 */
var dumpErr = function(err) {
    var m = {};
    for (var k in err) {
        m[k] = err[k];
    }
