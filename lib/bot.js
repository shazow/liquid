...
var eventHandlers = {
    'origin': {
        'trade': function(t) {
            logger.debug('[bot:origin] Trade: %j', t);
        ...
        'orderbook': function(orderbook) {
            ...
            logger.debug('[bot:origin] Orderbook: %d asks, %d bids', orderbook.asks.length, orderbook.bids.length);
        }
    },
    'remote': {
        'trade': function(t) {
            logger.debug('[bot:remote] Trade: %j', t);
        },
        'orderbook': function(orderbook) {
...
Bot.prototype.stop = function(callback) {
    ...
    var cleanup = function(callback) {
        ...
        util.removeListeners(bot.originExchange, eventHandlers.origin);
        util.removeListeners(bot.remoteExchange, eventHandlers.remote);
