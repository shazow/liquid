 ...
 * @param {number} premium Price multiplier when placing corresponding orders from remote to origin.
 */
var Bot = module.exports.Bot = function(originExchange, remoteExchange, premium) {
    this.state = 'idle';
...
Bot.fromConfig = function(config) {
    ...
    return new Bot(originExchange, remoteExchange, config.premium);
};
...
Bot.prototype.handleRemoteOrderbook = function(orderbook, callback) {
    ...
    var newOrders = order.aggregateOrders(orderbook.asks.concat(orderbook.bids), this.minValue, this.premium);
    var instructions = order.patchOrders(this.originExchange.getOrders(), newOrders);

    // XXX: async.parallel?
    this.originExchange.cancelOrders(instructions.cancel);
    this.originExchange.placeOrders(instructions.place, callback);
};
