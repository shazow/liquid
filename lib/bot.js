...
Bot.prototype.reset = function(callback) {
    ...
    async.series([
        ...
        function settleOutstanding(callback) {
            // Right now this is only used on start, when exchanges get ready
            // and populate their respective lastTransactions. Ultimately, this
            // stage can be used for failed orders and maybe other things?
...
Bot.prototype.handleRemoteOrderbook = function(orders, callback) {
    ...
    if (newOrders.length != budgetOrders.length) {
        logger.debug('[bot:remote] Pruned %d of %d aggregated orders due to low budget: [ASK: %d BTC, %d USD] [BID: %d BTC, %d USD]',
            newOrders.length-budgetOrders.length, newOrders.length,
