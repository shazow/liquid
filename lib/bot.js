...
var Bot = module.exports.Bot = function(originExchange, remoteExchange, options) {
    ...
    // TODO: Convert these to BigNumbers?
    this.stats = {
        numMatched: 0,
        valueMatched: 0,
        numPending: 0,
...
Bot.fromConfig = function(config) {
    ...
    var missingKeys = util.missingStringValues(config, allConfigKeys);
    if (!missingKeys.length) {
        logger.info('Bot created in LIVE mode. Real trades will occur on real exchanges.');
    } else {
        logger.info('Bot created in PRETEND mode due to missing API keys (%j). Orderbook will be watched but no trades will be placed.', missingKeys);
        config.pretend = true;
    }
...
Bot.prototype.stop = function(callback) {
    logger.info('Bot stopping:', this.stats);
    ...
    async.series([
        ...
        function checkRemoteOrders(callback) {
            ...
            orders.forEach(function(order) {
                bot.stats.valuePending += order.getValue().toNumber();
            });

            logger.warn('Found %d outstanding remote placed orders, value:', orders.length, bot.stats.valuePending);
            callback();
        }
    ], function(err, result) {
        if (err) {
...
Bot.prototype.handleOriginTrade = function(order, callback) {
    ...
    this.remoteExchange.placeOrders([newOrder], function(err, result) {
        ...
        bot.stats.valuePending += newOrder.getValue().toNumber();
...
Bot.prototype.handleRemoteTrade = function(order, callback) {
    ...
    this.stats.valueMatched += value.toNumber();
    this.stats.numPending--;
    this.stats.valuePending -= order.getValue().toNumber();
