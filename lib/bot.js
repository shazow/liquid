...
Bot.fromConfig = function(config) {
    ...
    if (config.live !== true) {
        ...
        var originExchange = new DummyExchange('DummyOrigin', 1000, DummyExchange.behaviors.origin);
        var remoteExchange = new DummyExchange('DummyRemote', 1000, DummyExchange.behaviors.remote);
...
Bot.prototype.handleRemoteTrade = function(order, callback) {
    logger.debug('[bot:remote] Trade:', order.toString());
};
