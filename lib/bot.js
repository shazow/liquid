...
Bot.prototype.handleRemoteOrderbook = function(orders, callback) {
    ...
    var budgetOrders = order.budgetOrders(newOrders) // XXX: , budget);
    ...
    if (newOrders.length != budgetOrders.length) {
        logger.debug('[bot:remote] Pruned %d/%d aggregated orders due to low balance.', newOrders.length-budgetOrders.length, newOrders.length);
    }
