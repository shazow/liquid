...
var patchOrders = module.exports.patchOrders = function(placedOrders, newOrders, tolerance) {
    ...
    // are already sorted.
    // TODO: This is may not be the optimal implementation in terms of reducing
    // thrashing with tolerance. To improve, some ideas:
    // 1. Do a second pass where we sort the patch by delta between placed and
    // cancelled orders, and remove any that are within tolerance.
    // 2. We can convert this into a graph optimization problem with a
    // cost function based on number of operations vs acceptable losses.
    ...
    while (true) {
        ...
        if (tolerance && cmp && placedOrder.type === newOrder.type) {
            // Similar order, check if within tolerance.
            var v = placedOrder.getValue();
            var relativeChange = v.minus(newOrder.getValue()).dividedBy(v).abs();
