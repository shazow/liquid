...
Order.prototype.similarTo = function(other) {
    var cmp = Number(this.rate.toFixed(1)) - Number(other.rate.toFixed(1));
    if (cmp == 0) cmp = Number(this.quantity.toFixed(7)) - Number(other.quantity.toFixed(7));
    if (cmp == 0 && this.type !== other.type) cmp = this.type === 'ASK' ? 1 : -1;
    return toCmp(cmp);
};
...
var patchOrders = module.exports.patchOrders = function(placedOrders, newOrders, tolerance) {
    ...
    var p = 0, c = 0;
    while (true) {
        if (p >= patch.place.length || c >= patch.cancel.length) {
            // Done pruning.
            break;
        }
