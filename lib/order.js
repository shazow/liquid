...
var patchOrders = module.exports.patchOrders = function(placedOrders, newOrders) {
    ...
    while (true) {
        ...
        if (placedOrder.type === newOrder.type && placedOrder.rate.eq(newOrder.rate) && placedOrder.quantity.eq(newOrder.quantity)) {
            // Same order, skip.
            p++;
            n++;
            continue;
        ...
        if (placedOrder.rate < newOrder.rate) {
            p++;
...
/**
 * Sort orders by (spread from mean, quantity, type, rate).
 *
...
var sortOrders = module.exports.sortOrders = function(orders, meanRate) {
    ...
    return orders.sort(function(a, b) {
        ...
        if (cmp == 0) cmp = a.quantity.comparedTo(b.quantity);
        if (cmp == 0 && a.type !== b.type) cmp = a.type === 'ASK' ? 1 : -1;
        if (cmp == 0) cmp = a.rate.comparedTo(b.rate);
        return cmp;
    });
};
