...
var budgetOrders = module.exports.budgetOrders = function(orders, budget, premium) {
    ...
    orders.forEach(function(order, i) {
        ...
        if (value.gt(budget.value) || quantity.gt(budget.quantity)) {
            return;
        }
 ...
 * Takes the minimum values available on both exchanges while accounting for
 * premiums.
 *
...
var getBudget = module.exports.getBudget = function(originBalance, remoteBalance, premium) {
    ...
    if (remoteBalance.quantity < budget.quantity) {
        budget.quantity = BigNumber(remoteBalance.quantity);
    }

    if (remoteBalance.value < budget.value) {
        budget.value = BigNumber(remoteBalance.value);
    }
