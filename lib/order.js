...
Order.prototype.toString = function() {
    return util.format('[Order #%s: %s %s @ $%s]', this.id, this.type, this.quantity.toFixed(), this.rate.toFixed());
};
...
// TODO: Does Transaction deserve its own module? It's fairly tightly coupled to
// orders.
/**
...
Transaction.prototype.toOrder = function(spread, invertType) {
    ...
    return new Order(o.id, o.type, o.quantity, rate);
};
...
/**
 * Given a list of recent transactions that we know about, figure out which
 * origin transactions were missed from remote and need to be patched.
 *
 * @param {Array.<object>} originTransactions
 * @param {Array.<object>} remoteTransactions
 * @param {object} remoteSpread Current market spread (as returned by getSpread)
 * @return {Array.<Order>} Orders to execute on remote exchange.
 */
var patchTransactions = module.exports.patchTransactions = function(originTransactions, remoteTransactions, remoteSpread) {
    // We really only care about the latest remoteTransaction for now.
    var lastRemote = remoteTransactions.length > 0 && remoteTransaction[0];
    var outstanding = originTransactions.filter(function(t) {
        if (lastRemote && t.comparedTo(lastRemote) == 0) return;
        return t;
    });

    return outstanding.map(function(t) {
        return t.toOrder(remoteSpread, true);
    });
};
