...
BitmeExchange.prototype.tick = function() {
    ...
    exchange.client.ordersOpen(function(err, response) {
        ...
        if (err) {
            logger.error('Failure during BitmeExchange polling:', err);
            return;
        }
        if (exchange.tickCount < tickCount) {
            // If this happens too much, we can switch the tick from
            // setInterval to a setTimeout loop.
            logger.warn('Out-of-order tick (%d steps) in BitmeExchange polling, skipping.', exchange.tickCount-tickCount);
            return;
        }
        ...
        if (exchange.pendingRequests > 0) {
            // Discard tick mid-completion because there are changes in-progress.
            exchange.debug('Skipping tick update of %d trades due to %d pending requests. (%d ticks skipped in a row.)', trades.length, exchange.pendingRequests, tickSkipStreak);
            return;
        }
...
BitmeExchange.prototype.cleanup = function(callback) {
    ...
    BitmeExchange.super_.prototype.cleanup.call(this, callback);
};
    ...
    this.requestMap(orders, function(order, callback) {
        ...
        exchange.client.orderCancel(order.id, function(err, result) {
            ...
            if (newOrder.quantity.toFixed(7) != order.quantity.toFixed(7)) {
                ...
                exchange.debug('Order partially executed before cancel, emitting a partial order:', deltaOrder.toStirng());
