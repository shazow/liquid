...
BitmeExchange.prototype.tick = function() {
    ...
    exchange.client.ordersOpen(function(err, response) {
        ...
        var using = exchange.resources.using('requests');
        if (using) {
            // Discard tick mid-completion because there are changes in-progress.
            exchange.debug('Skipping tick update of %d trades due to %d pending requests. (%d ticks skipped in a row.)', trades.length, using, tickSkipStreak);
            return;
        }
...
BitmeExchange.prototype.placeOrders = function(orders, callback) {
    ...
    this.requestMap(orders, function(order, callback) {
        ...
        exchange.resources.acquire('requests');
