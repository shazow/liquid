...
BitmeExchange.prototype.tick = function() {
    ...
    exchange.client.ordersOpen(function(err, response) {
        ...
        var trades = diffOrders(exchange.getOrders(), responseOrders, /* onlyMissing */ true);
        if (!trades.length) {
            // No new trades.
            return;
        }
...
BitmeExchange.prototype.ready = function(callback) {
    ...
    async.series([
        ...
        function loadTransactions(callback) {
            var transactions = [];
            // XXX: I think we can do this with just BTC and DEBIT vs CREDIT.
            async.each(['BTC', 'USD'], function(currencyCd, callback) {
                exchange.client.transactionsGet(currencyCd, 100, 'DESC', 1, function(err, response) {
                    if (err) {
                        logger.error('Failed to load %s transactions:', currencyCd, err.message);
                        callback && callback.call(exchange, err);
                        return;
                    }
                    var cleared = response.transactions.filter(function(t) {
                        if (t['transaction_category_cd'] != 'ORDER_ESCROW' || t['transaction_status_cd'] != 'CLEARED') return;
                    ...
                    transactions.push.apply(transactions, cleared);
                    callback && callback.call(exchange);
                });
            }, function sortTransactions(err) {
                if (err || transactions.length == 0) {
                    callback && callback.call(exchange, err);
                    return;
                }

                exchange.lastTransactions = transactions.sort(function(a, b) {
                    var a = a.created, b = b.created;
                    if (a > b) return 1;
                    if (a < b) return -1
                    return 0;
                });

                exchange.debug('Loaded last transactions from previous run:', transactions.length);
                callback && callback.call(exchange);
            });
        },
