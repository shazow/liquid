...
BitmeExchange.prototype.tick = function() {
    ...
    if (exchange.cancelPending) {
        // Don't poll for open orders if we're actively sending cancel requests
        return logger.debug('Skipping tick in BitmeExchange - %d cancelled orders pending', exchange.cancelPending);
    }

    exchange.tickStarted = true;
...
BitmeExchange.prototype.cancelOrders = function(orders, callback) {
    ...
    if (exchange.tickStarted) {
        // Don't send cancel order requests if we're actively polling for open orders
        var cancelWait = setInterval(function() {
            if (exchange.tickStarted) return; // Keep waiting
            clearInterval(cancelWait);
            exchange.cancelOrders(orders, callback);
        }, 100);
        return;
    }

    exchange.cancelPending += orders.length;
