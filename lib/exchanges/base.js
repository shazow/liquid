...
BaseExchange.prototype.cleanup = function(callback) {
    ...
    var cleanupWait = setInterval(function() {
        if (exchange.resources.using('requests')) {
            return; // Keep waiting
        }
 ...
 */
var Resources = module.exports.Resources = function() {
    this.counts = {};
};
 ...
 * Acquire a resource.
 *
 * @param {string} key Unique identifier of resource.
 * @param {boolean} exclusive Skip and return false if resource is already used.
 * @return {boolean|number} Returns either false if exclusive acquire failed, or truthy number of users.
 */
Resources.prototype.acquire = function(key, exclusive) {
 ...
 * @return {number}
 */
Resources.prototype.release = function(key) {
    var v = this.counts[key] || 0;
    v--;
    this.counts[key] = v;
    return v;
 ...
 * @return {number}
 */
Resources.prototype.using = function(key) {
