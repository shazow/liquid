...
var BaseExchange = module.exports.BaseExchange = function(pretend, delay) {
    ...
    if (this.tick && delay) {
        this.ticker = this.createTicker(this.tick.bind(this), this.constructor.name, delay);
    }
};
 ...
 * @param {number=} numAlerts Log warning if this many ticks are missed due to no exclusive lock. (Default: 120)
 */
var Ticker = module.exports.Ticker = function(fn, delay, resource, numAlert) {
    ...
    this.completed = 0;
    this.skipStreak = 0;
    this.skipAlert = numAlert || 120; // Log alert if we skip this many ticks in a row.
    this.loop = null;
};
