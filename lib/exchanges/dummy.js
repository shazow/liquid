 ...
 */
DummyExchange.behaviors = {
    'remote': function(exchange) {
        // Add and remove orders randomly.
        ...
        if (orders.length==0 || Math.random() > 0.5) {
            ...
            var type = {'asks': 'ASK', 'bids': 'BID'}[book];
            var quantity = Math.random().toFixed(3);
            var rate = (Math.random() * 100).toFixed(3);
            orders.push(new Order(id, type, quantity, rate));
        } else {
            // Remove
            orders.pop();
        };
    },
    'origin': function(exchange) {
