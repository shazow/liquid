 ...
 */
var BitstampExchange = module.exports = function(client, stream, pretend, delay) {
    ...
    exchange.rateLimiter.removeTokens(1, function() {
        exchange.requestLock.acquire(true /* exclusive */, function() {
            exchange.client.post('https://www.bitstamp.net/api/open_orders/', function(err, response, body) {
                ...
                if (emitTrades) {
                    exchange.loadTransactions(completed, function(err, orders) {
                        if (err) {
...
/**
 * Load recent transactions and find a recently completed order.
 *
 * @param {Array.<Order>} orders Find these orders in recent transactions.
 * @param {function=} callback Function called upon completion.
 */
BitstampExchange.prototype.loadTransactions = function(orders, callback) {
    var exchange = this;
    exchange.rateLimiter.removeTokens(1, function() {
        // Note: No request lock. Should be fine, right? Since this is stateless.
        exchange.client.post('https://www.bitstamp.net/api/user_transactions/', function(err, response, body) {
            var r = toResponse(response, err);
            var err = r.err, data = r.data;

            if (err) {
                logger.error('Failed to load Bitstamp transactions:', err);
                callback && callback.call(exchange, err);
                return;
            }
            ...
            for (var i in data) {
                var t = data[i];
                if (t.type !== 2) continue; // Skip deposits, etc.

                var order = foundIdx[t.order_id];
                if (order !== undefined) {
                    // Combine orders that were broken up into multiple transactions.
                    foundIdx[order.id] = order.combine(toOrder(t));
                    continue;
                }

                var order = idx[t.order_id];
                if (order === undefined) {
                    // Skip orders we don't care about.
                    continue;
                }

                delete idx[t.order_id];
                foundIdx[t.order_id] = toOrder(t);
            }

            var found = [];
            for (var k in foundIdx) {
                found.push(foundIdx[k]);
            }

            var notFound = [];
            for (var id in idx) {
                notFound.push(idx[id]);
            }

            if (exchange.pretend) {
                // Assume we found them, even though we probably didn't.
                notFound = [];
                found = found.concat(orders);
                logger.info('[exchange:bitstamp] Pretend loadTransactions:', orders.map(String));
            }

            if (notFound.length > 0) {
                err = new Error('Failed to find '+ notFound.length +' of '+ orders.length +' recent Bitstamp transactions.');
                err.found = found.map(String);
                err.notFound = notFound.map(String);
            }

            callback && callback.call(exchange, err, found);
        });
    });
};
...
var toOrder = BitstampExchange.toOrder = function(d) {
    ...
    if (d.order_id) {
        // Parse transactions, too.
        id = d.order_id
        type = ['BID', 'ASK'][~~(Number(d.btc) < 0)]; // Negative BTC = sell = ASK
        amount = Math.abs(Number(d.btc));
        price = d.btc_usd;
    }

    try {
        return new order.Order(id, type, amount, price, null, 'bitstamp');
    } catch (e) {
