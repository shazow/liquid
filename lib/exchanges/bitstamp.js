    ...
    async.series([
        ...
        function loadTransactions(callback) {
            exchange.client.post('https://www.bitstamp.net/api/user_transactions/', function(err, response, body) {
                ...
                data.every(function(transaction) { 
                    if (transaction.type != 2) return true; // Look for market trades.

                    exchange.lastTransactions = [transaction];
                    exchange.debug('Loaded last transaction from previous run:', transaction);
                    return false;
                });
